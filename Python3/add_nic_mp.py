...
def ResetVM(vm):
    cprint ("Quick reset cycle: ON then OFF ...", "cyan")
    VMpowerON(vm)
    time.sleep(5)
    # vm.PowerOff()
...
def replace_nics(vm):
    ...
    if no_disk > 1:
        ...
        '''
        # VirtualMachineBootOptionsBootableDevice["hd"]
        # VirtualMachineBootOptionsBootableDevice["bootDisk", "bootEthernet"]

        bootCfg = vim.vm.VirtualMachineBootOptionsBootableDevice["bootDisk"]
        bootTask = vm.ReconfigVM_Task(spec = bootCfg)
        tasks.wait_for_tasks(my_conn, [bootTask])
        '''
        ...
        if len(scsi_lst) > 1:
            ...
            print ("Reconfiguring vm " + vm.name)
...
def main():
    ...
    # if len(vmname) == 0:
    #     vm_name = input("VM name, eg. tec011 for ukpmtec011ccp8a, or TEM011 for UKPMTEM011CCP8A: ")
    #     vmname.append(vm_name)

    if len(vmname) == 0:
        vmlist_file = "vmlist2.txt"
        try:
            ...
            print (e)
            vm_name = input("VM name, eg. ukpmtec011ccp8a, or q for none: ")
